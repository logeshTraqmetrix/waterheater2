...
const ViewSpares = () => {
    ...
    const handleEditSpares = (spares) => {
        ...
        setEditFormData({
            ...
            Warranty: spares.Warranty,
            Price: spares.Price,
            BarCodeValue: spares.BarCodeValue
        });
        setShowEditModal(true);
    };
    ...
    const handleEditSubmit = () => {
        ...
        axios.get(`/server/waterheater_1_function/getfilterticket?search=${encodeURIComponent(JSON.stringify({
            ...
            column: 'Spares_Id',
            value: editFormData.ROWID
        }))}`)
            .then((response) => {
                console.log('response from technician stock table', response.data)

                let tstockData = response.data

                tstockData.forEach((item) => {
                    if (item.technician_stocks.Spares_Id === editFormData.ROWID) {
                        item.technician_stocks.Spares_Name = editFormData.Material_Name;
                        item.technician_stocks.Price = editFormData.Price;
                        item.technician_stocks.BarCodeValue = editFormData.BarCodeValue;
                    }
                });

                // Extract updated data
                let technicianStockPayload = tstockData.map(item => ({
                    ROWID: item.technician_stocks.ROWID,
                    Spares_Name: item.technician_stocks.Spares_Name,
                    Price: item.technician_stocks.Price,
                    BarCodeValue: item.technician_stocks.BarCodeValue
                }));
                ...
                axios.put('/server/waterheater_1_function/update-technician-stock', { data: technicianStockPayload })
                    .then((response) => {
                        ...
                        if (response.status === 200) {
                            axios.put('/server/waterheater_1_function/updatespares', {
                                    data: [editFormData]
                                })
                                .then((res) => {
                                    console.log('Spares updated successfully');
            ...
            .catch((error) => {
                console.log('error in getting technician stock data', error)
            })
    };
        ...
        <div className='container'>
            ...
            <Modal show={showModal} onHide={handleCloseModal}>
                ...
                <Modal.Body>
                    {selectedSpares && (
                        <>
                            <p>Material Name: {selectedSpares.Material_Name}</p>
                            <p>Warranty: {selectedSpares.Warranty}</p>
                            <p>Consumed Qty: {selectedSpares.Consumed_Qty}</p>
                            <p>Price: {selectedSpares.Price}</p>
                            <p>Available Qty: {selectedSpares.Available_Qty}</p>
                            <p>Inward Qty: {selectedSpares.Inward_Qty}</p>
                            <p>Barcode Value: {selectedSpares.BarCodeValue}</p>
                        </>
                    )}
                </Modal.Body>
                <Modal.Footer>
                    ...
                    <Form>
                        ...
                        <Form.Group controlId="formAvailableQty">
                            ...
                            <Form.Control
                                ...
                                placeholder="Enter available qty"
                                name="BarCodeValue"
                                value={editFormData.BarCodeValue}
                                onChange={handleEditInputChange}
                            />
                        </Form.Group>
