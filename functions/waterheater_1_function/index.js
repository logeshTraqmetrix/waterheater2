...
// 	// Function defined within the route handler
// 	function getDataFromCatalystDataStore(catalystApp, table, column, value) {
// 	    return new Promise((resolve, reject) => {
// 	        // Queries the Catalyst Data Store table
// 	        catalystApp.zcql().executeZCQLQuery(`SELECT * FROM ${table} WHERE ${column} ='${value}'`)
// 	            .then(queryResponse => {
// 	                resolve(queryResponse);
// 	            })
// 	            .catch(err => {
...
// 	getDataFromCatalystDataStore(catalystApp, 'ticket_table', 'Ticket_Date', formattedDate)
// 	    .then(detail => {
// 	        res.send(detail);
// 	    })
// 	    .catch(err => {
...
//         return new Promise((resolve, reject) => {
//             // Example of combining two queries
//             let promises = [];

//             // First query
//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table WHERE Status ='Closed'`)); //2

//             // Second query
//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table`)); //1

//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table WHERE Status = 'Created Ticket'`)); //3

//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table WHERE Status = 'Technician Assigned'`)); //4

//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table WHERE Closed_Date = '${date}'`)); //5

//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT COUNT(ROWID) AS ID FROM ticket_table WHERE Scheduled_Date = '${date}'`)); //6

//             promises.push(catalystApp.zcql().executeZCQLQuery(`select sum(Expense_Cost) from list_of_spares where Warranty = 'yes'`)); //7

// 			promises.push(catalystApp.zcql().executeZCQLQuery(`select sum(SubTotal) from list_of_spares where Warranty = 'no'`)); //8

// 			promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT CREATEDTIME, Warranty_Available FROM ticket_table ORDER BY CREATEDTIME`)); //9

// 			promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT Closed_Date FROM ticket_table WHERE Closed_Date IS NOT NULL ORDER BY Closed_Date`)); //10

// 			promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT Invoice_Date, Warranty, SUM(SubTotal), SUM(Expense_Cost) FROM list_of_spares GROUP BY Invoice_Date,Warranty`)); //10
...
//     // Usage of the combined function
//     getDataFromCatalystDataStore(catalystApp, formattedDate)
//         .then(results => {
			            
//             res.send({ ticketClosed: results[0], total: results[1], totalUnAssigned: results[2], totalAssigned: results[3], todayClosedTicket: results[4], todayAssignedTicket: results[5], warrantyCost: results[6], nonWarrantyCost: results[7], warrantyAndNonWarranty: results[8], ClosedOnEachDate: results[9],costofWarrantyAndNonWarranty: results[10] });
//         })
//         .catch(err => {
//             console.log(err, "error in getting data from filter condition");
//             res.status(500).send(err);
...
//             promises.push(catalystApp.zcql().executeZCQLQuery(`SELECT Invoice_Date, Warranty, SUM(SubTotal), SUM(Expense_Cost) FROM list_of_spares GROUP BY Invoice_Date,Warranty`));
